...
/* Programmatically caluclate array size */ 
...
// test this
void ftimsPulseFreq(int startFreq, int endFreq, int sweepTime, int steps){
    int period = 1000000 / startFreq; // Calculate the period of the starting frequency
    int stepSize = (endFreq - startFreq) / steps; // Calculate the frequency step size
    int stepTime = sweepTime / steps; // Calculate the time for each step
    
    for (int i = 0; i < steps; i++) {
      // Have to write the frequency before you start writing (needs testing)
      //analogWriteFrequency(TX_OUTPUT, startFreq); // Write the inital starting frequency 
...
// Main loop
void loop() { 
  ftimsPulseFreq(startFreq, endFreq, sweepTime, steps);
